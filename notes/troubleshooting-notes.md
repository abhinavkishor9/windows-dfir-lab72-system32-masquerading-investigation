# Troubleshooting Notes – System32 Masquerading Investigation
...
|---|---|---|
| Many `svchost.exe` processes | Normal Windows behavior | Investigate executable path |
| Lab process not visible later | Process may have exited | Use Sysmon Event ID 1 |
| Event 11 only showed file creation | Event 11 is file creation telemetry | Correlate with Event ID 1 |
| Wazuh timestamp slightly later | Telemetry processing delay | Correlate by path and event context |
| Signature was valid | Original binary was Microsoft-signed | Validate path and binary identity |
| Filename said `svchost.exe` | File was intentionally renamed | Compare metadata and hash |
| Hash differed from legitimate svchost | Different binary | Compare against known source |
