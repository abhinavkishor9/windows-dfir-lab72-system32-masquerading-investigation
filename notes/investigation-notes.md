# Investigation Notes – System32 Masquerading

## Investigation Overview

This investigation examines a Windows executable named `svchost.exe` that was executed from an unexpected directory.

The objective was to determine whether the file was the legitimate Windows Service Host binary or another executable using a trusted process name.

The investigation followed the principle:

> **Do not trust the filename. Validate the binary and its execution context.**
...
- `svchost.exe` was present outside the legitimate System32 directory.
- The lab executable was created at `11-09-2026 07:41:12`.
- Sysmon Event ID 11 recorded the file creation.
- Sysmon Event ID 1 recorded process execution at `08:20:29`.
- Wazuh observed execution from the lab directory.
- The SHA256 matched the original `notepad.exe`.
- The SHA256 did not match legitimate `svchost.exe`.
- Wazuh identified the file description as `Notepad`.
- The binary retained a valid digital signature.
- The activity was intentionally generated for this DFIR lab.
...
These conclusions were not part of the controlled scenario.
...
The evidence demonstrates how a trusted process name can be misleading.

The binary's actual identity was established through hash and metadata analysis rather than its filename.

---

## Analyst Takeaway

When investigating a suspicious Windows process, use:
...
A trusted filename should trigger validation, not automatic trust.
